...
import TwoSample_Equal_GetSolution from './TwoSample_Equal_GetSolution';
import TwoSample_Equal_Solution from './TwoSample_Equal_Solution';
import TwoSample_Equal_Theory from './TwoSample_Equal_Theory';


const TwoSample_Equal = () =>{
...
const [getSolutionDataObject,setSolutionDataObject] = useState({});
    ...
    const getSolutionData = (selected,changeHypothesis1,changeHypothesis2,numProbability,changeCriticalPValue, numDifference, numTestStatistic,choiceConfidenceInterval
        , numDecimalPoint, numPopulationMean, numMeanDifference, numSampleAvg, numSampleAvg1, numSampleSize, numSampleSize1,numSampleSD,numSampleSD1,numMulValue) =>{
                ...
                <div classNameName="col-lg-12 col-12 multitabs">
                    ...
                    <div className="tab-content">
                        <div className={`tab-pane ${showcode == false ? "active" : " "}`} id="tabs-41" role="tabpanel">
                            <TwoSample_Equal_GetSolution sendtoparent={sendtoparent} getSolutionData={getSolutionData} />
                        </div>
                        {showcode == true ? <div className={`tab-pane ${showcode == true ? "active" : " "}`} id="tabs-42" role="tabpanel">
                            <TwoSample_Equal_Solution getSolutionDataObject={getSolutionDataObject} />
                        </div> : null}
                        <div className="tab-pane" id="tabs-43" role="tabpanel">
                            <TwoSample_Equal_Theory />
                        </div>
                    </div>
                </div>
            </div>
...
export default TwoSample_Equal;