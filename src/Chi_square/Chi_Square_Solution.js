...
const Chi_Square_Solution = (props) => {
  
  /* --------------------  Left Side -----------------*/
  
  const [Chi_Square, setChi_Square] = useState(0);

    useEffect(() => {
    let result = jStat.chisquare.inv(
    ...
    setChi_Square(result);
  });

  /* --------------------  Right Side -----------------*/
  
  const [Chi_Square1, setChi_Square1] = useState(0);

    useEffect(() => {
    let result1 = jStat.chisquare.inv(1 -
      props.getSolutionDataObject.numSign,
      props.getSolutionDataObject.numDof
    );
    ...
    setChi_Square1(result1);
  });

   /* --------------------  Two Tailed -----------------*/
  
   const [Chi_Square2, setChi_Square2] = useState(0);

   useEffect(() => {
   let result2 = jStat.chisquare.inv(1 -
   ...
   setChi_Square2(result2);
 });

 /* --------------------  Two Tailed 1 -----------------*/
  
 const [Chi_Square3, setChi_Square3] = useState(0);

 useEffect(() => {
 let result3 = jStat.chisquare.inv(
   props.getSolutionDataObject.numSign /2,
   props.getSolutionDataObject.numDof 
 );
                ...
                <div className="col-lg-12 col-12 ">
                  ...
                  <div className="soln-box">
                    <p>
                      ...
                      tailed χ2-distribution with{" "}
                      {props.getSolutionDataObject.numDof} degrees of freedom at{" "}
                      {props.getSolutionDataObject.numSign} level of
                      significance & the critical value is :
                    </p>
                    <div className="criticalbox">
                      <p>Critical value (χ2c) :</p>
                      <p>2.1318</p>
                    </div>
                    <div className="criticalbox">
                      <p>Critical region:</p>
                  ...
                  <div className="sigsolution">
                    <p className="mt-2">
                      = CHISQ.INV.RT (Significance Level (α), deg_freedom)
                    </p>
                    <p className="mt-2">
                      = CHISQ.INV.RT ({props.getSolutionDataObject.numSign},
                      {props.getSolutionDataObject.numDof})
                    </p>
                    <p className="mt-2">= {Chi_Square} and {Chi_Square1} and {Chi_Square3}, {Chi_Square2}</p>
                  </div>
                  <div class="sol-para">
                    <p>
                      Here, the test is {props.getSolutionDataObject.tailed}
                      ...
                      {props.getSolutionDataObject.numDof} degrees of freedom at
                      {props.getSolutionDataObject.numSign} level of
                      significance & the critical value is {Chi_Square}
                    </p>
                  </div>
                </div>
              </div>
...
export default Chi_Square_Solution;
