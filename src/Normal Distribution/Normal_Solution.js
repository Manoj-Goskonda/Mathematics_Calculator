...
import Normal6 from '../assets/Images/Normal6.png';
import Normal7 from '../assets/Images/Normal7.png';
import img1 from '../assets/Images/image98.png'

const Normal_Solution = (props) => {

 /* ------------ = Equal ----------- */
    const [Success, setSuccess] = useState(0);  

    useEffect(() => {
    let result = jStat.normal.pdf( 
...
useEffect(() => {
let result3 = 1 - jStat.normal.cdf( 
    parseFloat(props.getSolutionDataObject.numEqual),
    parseFloat(props.getSolutionDataObject.numTrails),
    parseFloat(props.getSolutionDataObject.numProbability)
)
...
useEffect(() => {
let result4 = 1 - jStat.normal.cdf( 
    parseFloat(props.getSolutionDataObject.numEqual),
    parseFloat(props.getSolutionDataObject.numTrails),
    parseFloat(props.getSolutionDataObject.numProbability)
)
...
useEffect(() => {
  let result5 = 1 - jStat.normal.cdf(
    parseFloat(props.getSolutionDataObject.numEqual),
    parseFloat(props.getSolutionDataObject.numTrails),
    parseFloat(props.getSolutionDataObject.numProbability)
    );
...
useEffect(() => {
  let result6 = Success + 1 - jStat.normal.cdf(
    parseFloat(props.getSolutionDataObject.numEqual),
    parseFloat(props.getSolutionDataObject.numTrails),
    parseFloat(props.getSolutionDataObject.numProbability)
    );
                        ...
                        <div className="input container">
                            <div className="row justify-content-center">
                                <div className="col-lg-12 col-12">
                                    ...
                                    <p> <img src={Normal1} /> </p><br />
                                        <div className="formula">
                                        <p><img src={Normal2} /></p><br />
                                        <p><img src={Normal3} /></p><br />
                                        <p><img src={Normal4} /></p><br />
                                        </div>  
                                    <p> <img src={Normal5} /> </p><br />
                                    <div className="formula">
                                        <p><img src={Normal6} /></p><br />                                    
                                    </div> 
                                    <p className="formula" style={{fontSize:"30px"}}> ={ `P(Z<0.5) - P(Z< - 1.25)`}</p> 
                                    <p>Use the standard normal table to conclude that :</p> 
                                    <p className="formula" style={{fontSize:"30px"}}> = 0.923 - 0.32</p>  
                                    <p className="formula" style={{fontSize:"20px"}}> = {Success}</p>
                                    <br /><br /> 
                                    <p style={{marginLeft:'20px'}}>Therefore, based on the information provided, it is concluded that P ( 15 {"<"} X {'<'} 22 ) = {Success}.</p>
                                    <br></br>
                                    <img src={Normal7} className="theory-graph-img" />
                                </div>
                            </div>
                        </div>
                    </div>
...
export default Normal_Solution;
