...
import OneSample_ZTest_Praportion_GetSolution from './OneSample_ZTest_Praportion_GetSolution';
import OneSample_Ztest_Praportion_Solution from './OneSample_ZTest_Praportion_Solution';
import OneSample_ZTest_Praportion_Theory from './OneSample_ZTest_Praportion_Theory';


const  OneSample_ZTest_Praportion = () =>{
...
const getSolutionData = (selected,changeHypothesis1,changeHypothesis2,numProbability,changeCriticalPValue,numTestStatistic,choiceConfidenceInterval
    ,numGroupName,numDecimalPoint,numPopulationMean,numSampleAvg, numSampleSize,numSampleSD,numMulValue) =>{
...
dataobj.numGroupName = numGroupName;
                ...
                <div classNameName="col-lg-12 col-12 multitabs">
                    ...
                    <div className="tab-content">
                        <div className={`tab-pane ${showcode == false ? "active" : " "}`} id="tabs-41" role="tabpanel">
                            < OneSample_ZTest_Praportion_GetSolution sendtoparent={sendtoparent} getSolutionData={getSolutionData} />
                        </div>
                        {showcode == true ? <div className={`tab-pane ${showcode == true ? "active" : " "}`} id="tabs-42" role="tabpanel">
                            < OneSample_Ztest_Praportion_Solution getSolutionDataObject={getSolutionDataObject} />
                        </div> : null}
                        <div className="tab-pane" id="tabs-43" role="tabpanel">
                            < OneSample_ZTest_Praportion_Theory />
                        </div>
                    </div>
                </div>
            </div>
...
export default OneSample_ZTest_Praportion;