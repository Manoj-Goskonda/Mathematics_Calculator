...
import TwoSample_ZTest_Praportion_GetSolution from './TwoSample_ZTest_Praportion_GetSolution';
import TwoSample_ZTest_Praportion_Solution from './TwoSample_ZTest_Praportion_Solution';
import TwoSample_ZTest_Praportion_Theory from './TwoSample_ZTest_Praportion_Theory';


const  TwoSample_ZTest_Praportion = () =>{
...
const [getSolutionDataObject,setSolutionDataObject] = useState({});
    ...
    const getSolutionData = (selected,changeHypothesis1,changeHypothesis2,numProbability,changeCriticalPValue, numDifference, numTestStatistic,choiceConfidenceInterval
        , numDecimalPoint, numPopulationMean, numMeanDifference, numSampleAvg, numSampleAvg1,numPraportion,numPraportion1, numSampleSize, numSampleSize1,numSampleSD,numSampleSD1,numMulValue) =>{
    ...
    ,numDecimalPoint,numPopulationMean,numMeanDifference,numSampleAvg,numSampleAvg1, numSampleSize,numPraportion,numPraportion1, numSampleSize1,numSampleSD,numSampleSD1,numMulValue);
    let dataobj = {};
    ...
    dataobj.numMeanDifference = numMeanDifference;
    dataobj.numSampleAvg = numSampleAvg;
    dataobj.numPraportion = numPraportion;
    dataobj.numPraportion1 = numPraportion1;
                ...
                <div classNameName="col-lg-12 col-12 multitabs">
                    ...
                    <div className="tab-content">
                        <div className={`tab-pane ${showcode == false ? "active" : " "}`} id="tabs-41" role="tabpanel">
                            < TwoSample_ZTest_Praportion_GetSolution sendtoparent={sendtoparent} getSolutionData={getSolutionData} />
                        </div>
                        {showcode == true ? <div className={`tab-pane ${showcode == true ? "active" : " "}`} id="tabs-42" role="tabpanel">
                            < TwoSample_ZTest_Praportion_Solution getSolutionDataObject={getSolutionDataObject} />
                        </div> : null}
                        <div className="tab-pane" id="tabs-43" role="tabpanel">
                            < TwoSample_ZTest_Praportion_Theory />
                        </div>
                    </div>
                </div>
            </div>
...
export default TwoSample_ZTest_Praportion;